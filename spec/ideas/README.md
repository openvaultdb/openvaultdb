---
format: https://specscore.md/ideas-index-specification
---

# Ideas

Pre-spec one-pagers. Each Idea is a lint-clean problem-direction-MVP one-pager that may later promote into one or more SpecScore Features under [`features/`](../features/README.md).

## Index

| Idea | Status | Date | Owner | Promotes To |
|------|--------|------|-------|-------------|
| [ovdb-onboarding-and-configuration](ovdb-onboarding-and-configuration.md) | Specified | 2026-09-17 | alex | ai-agent-skills, configuration-parity, database-context-navigation, database-setup-and-providers, explore-data-handoff, first-run-onboarding, local-server-and-web-console, telemetry-consent, todo-demo |

## Open Questions

None at this time.

---
*This document follows the https://specscore.md/ideas-index-specification*
